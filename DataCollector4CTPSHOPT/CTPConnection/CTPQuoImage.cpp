...
#include "../DataCollector4CTPSHOPT.h"
...
void CTPQuoImage::BuildBasicData()
{
	...
	tagSHOptionMarketStatus_HF137	tagStatus = { 0 };
	...
	{
		tagSHOptionKindDetail_LF136		tagKind = { 0 };

		::strncpy( tagKind.KindName, "指数保留", 8 );
		tagKind.PriceRate = 0;
		tagKind.LotFactor = 0;
		m_mapRate[m_mapRate.size()] = tagKind.PriceRate;
		QuoCollector::GetCollector()->OnImage( 136, (char*)&tagKind, sizeof(tagKind), true );
	}
	{
		tagSHOptionKindDetail_LF136		tagKind = { 0 };

		::strncpy( tagKind.KindName, "上海期指", 8 );
		tagKind.PriceRate = 2;
	...
	}
	{
		...
		::strncpy( tagKind.KindName, "上海期权", 8 );
		tagKind.PriceRate = 2;
		tagKind.LotFactor = 100;
		m_mapRate[m_mapRate.size()] = tagKind.PriceRate;
		QuoCollector::GetCollector()->OnImage( 136, (char*)&tagKind, sizeof(tagKind), true );
	...
	QuoCollector::GetCollector()->OnImage( 135, (char*)&tagMkInfo, sizeof(tagMkInfo), true );
	QuoCollector::GetCollector()->OnImage( 137, (char*)&tagStatus, sizeof(tagStatus), true );
}
...
void CTPQuoImage::OnRspQryInstrument( CThostFtdcInstrumentField *pInstrument, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast )
{
	...
	if( NULL != pInstrument )
	{
		...
		{	///< 判断为是否需要过滤的商品
			...
			QuoCollector::GetCollector()->OnImage( 139, (char*)&tagSnapLF, sizeof(tagSnapLF), bIsLast );
			QuoCollector::GetCollector()->OnImage( 140, (char*)&tagSnapHF, sizeof(tagSnapHF), bIsLast );
			QuoCollector::GetCollector()->OnImage( 141, (char*)&tagSnapBS, sizeof(tagSnapBS), bIsLast );
		}
	}
